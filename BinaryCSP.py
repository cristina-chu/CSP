...
def consistent(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints: 	#look at all binary constraints
		if (constraint.affects(var)):			#if the constraint affect the variable at hand, look at it
			if (assignment.isAssigned(constraint.otherVariable(var))):	#if other variable assigned, check 
				other = constraint.otherVariable(var)			#get other variable
				val2 = assignment.assignedValues[other]
				
				if (not constraint.isSatisfied(value, val2)):	#if constraint not satisfied, false
					return False
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for value in orderValuesMethod(assignment,csp, var):
		if consistent(assignment, csp, var, value):	
			...
			if (inferences is not None):   #no failure in inferences
				#add inferences to assignment
				for inf in inferences:
					if inf[1] in assignment.varDomains[inf[0]]:
			...
			assignment.assignedValues[var] = None		#remove var=value
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	#get var with less values in varDomains

	#what to do with already assigned variables?

	#using the first var --> might need to check if assigned(?)
	#while x.isAssigned() yadda yadda
	for i in domains.items():
		if assignment.isAssigned(i[0])==False: 
			minVals = len(i[1])
			nextVar = i[0]
			break	

	for x in domains:							#for all variables 
		if assignment.isAssigned(x)==False:					#if variable not assigned
			numVals = len(domains[x])
			
			if numVals<minVals:			#if less values, then take it
				nextVar = x
				minVals = numVals

			elif numVals==minVals:	#if same value, degree heuristics (choose variable with most constraints on remaining variables)
				#how do you get constrains/variables affected

				countX = 0
				countY = 0

				for con in csp.binaryConstraints:
					if con.affects(x) and assignment.assignedValues[con.otherVariable(x)]==None:	#constraint affects x and other variable not assigned
						countX = countX+1

					elif con.affects(nextVar) and assignment.assignedValues[con.otherVariable(nextVar)]==None: 	#constrain affects nextVar and other variable not assigned
						countY = countY+1
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	domains = assignment.varDomains.items()	#list of (key,value) pairs 
	countVals = []

	for val in values:				#look at possible values for the variable
		count = 0

		for x in domains: 			#look at rest of values domains
			if val in x[1]: 		#if value in domain, count it 
				count += 1

		countVals.append((val,count))

	countVals = sorted(countVals, key=lambda value: value[1])

	resultVals = map(lambda x: x[0], countVals)
...
def forwardChecking(assignment, csp, var, value):
	...
	for x in csp.binaryConstraints:		#get all constraints that affect variable at hand
		if x.affects(var):
			if value in assignment.varDomains[x.otherVariable(var)]:		#if value in domain of other variable, take delete it
				inferences.add((x.otherVariable(var), value))
				assignment.varDomains[x.otherVariable(var)].remove(value)

				if len(assignment.varDomains[x.otherVariable(var)])==0:	 	#if no more values left, failure
					#add inferences back to assignments and return None
					for i in inferences:
						assignment.varDomains[i[0]].add(i[1])
...
def revise(assignment, csp, var1, var2, constraint):
	...
	for y in assignment.varDomains[var2]:			#go through list of possible values in domain of var1 - consistent values
		for x in assignment.varDomains[var1]: 		#go through list of possible values in domain of var2
			if constraint.isSatisfied(x,y):			#if satisfied, done
				revised = True
				break

		if revised == False:						#if not satisfied, remove
			inferences.add((var2, y))
	...
	if len(inferences) != 0:
		for var,val in inferences:
			...
			if len(assignment.varDomains[var])==0: 		#inconsistent
				#add all the inferences back
				for varBack, valBack in inferences:
					if valBack not in assignment.varDomains[varBack]:
						assignment.varDomains[varBack].add(valBack)
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	#start by populating with all arc CONTAINING var

	queue = deque()

	#add all constrains that affect var
	for con in csp.binaryConstraints: 		
		if con.affects(var):
			queue.append((con.otherVariable(var), var, con))

	while len(queue)!=0:
		vari,varj,constraint = queue.popleft()

		consistent = revise(assignment, csp, varj, vari, constraint) 	#getting inferences from revise

		if consistent==None: 
			#assignments broke, so add inferences back
			for inf in inferences:
			...
			for i in consistent:
				if i not in inferences:
					inferences.add(i)

			for con2 in csp.binaryConstraints:
				if con2.affects(vari) and con2.otherVariable(vari)!=varj: 
					queue.append((con2.otherVariable(vari), vari, con2))
...
def AC3(assignment, csp):
	...
	#add all constrains that affect var
	for con in csp.binaryConstraints: 		
		queue.append((con.var1, con.var2, con))
		queue.append((con.var2, con.var1, con))
	...
	while len(queue)!=0:
		...
		if consistent==None: 
			...
			for i in consistent: 			#add inferences from revise to inferences 
				if i not in inferences:
					inferences.add(i)

					#if i[1] in assignment.varDomains[i[0]]: 		#remove values from inferences from revise
					#	assignment.varDomains[i[0]].remove(i[1])

			for con2 in csp.binaryConstraints:
				if con2.affects(vari) and con2.otherVariable(vari)!=varj: 
					queue.append((vari, con2.otherVariable(vari), con2))
					queue.append((con2.otherVariable(vari), vari, con2))
